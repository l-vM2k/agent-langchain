...
from ayaka.config import ModelConfig
...
def create_llm(cfg: ModelConfig) -> BaseChatModel:
    ...
    Routing:
      - AYAKA_FAKE_LLM=1            -> FakeChatModel (offline, no network)
      - provider_code == "ollama"    -> ChatOllama
      - anything else (qianwen/deepseek/openai/custom_api...)
                                     -> ChatOpenAI (OpenAI-compatible)
    """
    if os.environ.get("AYAKA_FAKE_LLM") == "1":
        from ayaka.llm.fake import FakeChatModel
