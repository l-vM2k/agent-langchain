...
from ayaka.soul.models import PersonaType, SoulPerception, SoulState, SoulTurnTrace
...
class StateStrategyParams:
    """State 策略参数(从 strategies.yaml 加载)"""
    ...
    def decay_rate(self, minutes: float) -> float:
        """clamp(min + minutes/divisor * slope, min, max)"""
        if minutes <= 0:
