"""Agent lifecycle manager — lazy-load per device_sn, mirroring Java AgentManager."""
...
from xiaowei.config import AgentConfig, ConfigLoader
from xiaowei.llm import create_llm
from xiaowei.memory.reme_store import ReMeMemoryStore
from xiaowei.soul.models import PersonalityDimensions, PersonalityProfile, PersonaType
from xiaowei.soul.orchestrator import (
    SoulOrchestrator,
    SoulTurnFinalizer,
    SoulTurnPreparer,
)
...
from xiaowei.soul.state_machine import (
...
class AgentManager:
    ...
    Mirrors the Java AgentManager: lazy creation, one instance per device.
    Each agent bundles its own soul pipeline (state store / memory store /
    ReMe workspace) so contacts on the same device stay isolated.
    """
    ...
    def _build(self, device_sn: str) -> CompiledStateGraph:
        ...
        from xiaowei.agent.graph import build_agent_graph
    ...
    def _build_personality(self, agent_cfg: AgentConfig, device_sn: str) -> PersonalityProfile:
        """从 agent 配置解析人格 — 对应 loadConfig(含种子复制)。"""
    ...
    def _build_orchestrator(self, agent_cfg: AgentConfig, device_sn: str) -> SoulOrchestrator:
        """组装灵魂链(Preparer + Finalizer)— 对应 SoulOrchestrator 装配。"""
