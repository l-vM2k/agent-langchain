...
Mirrors the Java middleware chain as a 6-node graph:

    START -> prepare_soul -> retrieve_reme -> inject_soul_prompt
            -> react_agent -> record_reme -> finalize_soul -> END

- prepare_soul:     SoulTurnPreparer (personality/emotion/behavior/memory/prompt)
...
- finalize_soul:    SoulTurnFinalizer (state machine + memory extraction)
"""
...
class AgentState(TypedDict):
    """Graph state — mirrors the Java RuntimeContext + SoulContext bundle."""
...
def _make_prepare_soul(orchestrator: SoulOrchestrator, personality: PersonalityProfile):
    """prepare_soul 节点 — SoulTurnPreparer.prepare"""
...
def _make_retrieve_reme(reme_store: ReMeMemoryStore):
    """retrieve_reme 节点 — ReMeMemoryMiddleware.retrieve
...
def _make_record_reme(reme_store: ReMeMemoryStore):
    """record_reme 节点 — ReMeMemoryMiddleware.record(本轮对话写入)"""
...
def _make_finalize_soul(orchestrator: SoulOrchestrator):
    """finalize_soul 节点 — SoulTurnFinalizer.finalize(状态机 + 记忆摄取)"""
...
def _last_user_text(messages: list[BaseMessage]) -> str:
    """取最后一条 user 消息文本 — ReMeMemoryMiddleware.findLastUserMessage"""
...
def _build_turn_trace(messages: list[BaseMessage]) -> SoulTurnTrace:
    """从消息流统计工具调用轨迹 — 对应 SoulTurnTrace 的 toolSuccess/Failure 计数。"""
    tool_call_count = 0
