"""ReMe long-term memory store — mirrors ReMeMemoryStore.java + ReMeMemoryMiddleware.
...
from xiaowei.memory.anonymous import is_anonymous

# 检索注入的包装模板 — ReMeMemoryMiddleware.wrap
WRAP_TEMPLATE = """
...
class ReMeMemoryStore:
    ...
    def _workspace_id(self, device_sn: str, contact_id: str) -> str:
        """workspace_id = deviceSn_contactId — ReMeMemoryStore.resolveMemory"""
    ...
    async def record(
        ...
        """写入一轮记忆。匿名跳过;失败静默 — ReMeMemoryStore.record"""
        if is_anonymous(contact_id):
            return
