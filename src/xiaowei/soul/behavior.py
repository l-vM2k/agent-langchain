"""Behavior signal generation — mirrors BehaviorSignalGenerator.java.
...
(config/personality/strategies.yaml), mirroring WarmCompanionStrategies /
RationalAssistantStrategies / QuietGuardianStrategies.
"""
...
def _normalize_signed(value: float) -> float:
    """-1~1 → 0~1 — BehaviorSignalGenerator.normalizeSigned"""
...
class BehaviorSignalGenerator:
    ...
    def generate(
        ...
        """生成 6 维行为信号 — BehaviorSignalGenerator.generate"""
    ...
    # ---------- 关系档位推断 — BehaviorSignalGenerator.relationshipTone ----------
    ...
    def _weighted(*pairs: tuple[float, float]) -> float:
        """加权求和 — BehaviorSignalGenerator.weighted"""
