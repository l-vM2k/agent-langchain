"""Soul prompt composition — mirrors SoulPromptComposer.java.
...
def _band(value: float) -> str:
    """均分四档: <0.25 LOW, <0.5 MID_LOW, <0.75 MID_HIGH, 否则 HIGH — SoulPromptComposer.band"""
    if value < 0.25:
        return "LOW"
    if value < 0.5:
        return "MID_LOW"
...
def _fallback_label(value: float) -> str:
    """兜底标签(表内未配置该维度档位时) — SoulPromptComposer.fallbackLabel"""
    if value < 0.4:
        return "偏低"
    if value < 0.7:
        return "适中"
...
def _normalize_signed(value: float) -> float:
    """-1~1 → 0~1 — SoulPromptComposer.normalizeSigned"""
    return max(0.0, min(1.0, (value + 1) / 2))


class SoulPromptComposer:
    """动态人格 Prompt 生成器 — SoulPromptComposer.java"""
    ...
    def compose(
        ...
        """组装人格运行时系统提示 — SoulPromptComposer.compose"""
    ...
    def _dimension_section(self, title: str, dims, personality, soul, signal) -> str:
        """遍历一组维度,按各自当前值取档位文案 — appendDimensionSection"""
        lines = [f"{title}:\n"]
    ...
    def _relationship_tone_prompt(self, tone: RelationshipTone) -> str:
        """关系档位文案(离散枚举,文案保留在代码内) — appendRelationshipTonePrompt"""
    ...
    def _contact_section(self, contact: ContactProfile) -> str:
        """【关于对方】联系人档案段 — renderContactSection"""
        lines = ["\n【关于对方】\n"]
    ...
    def _resolve_text(self, dimension: str, value: float) -> str:
        """解析某维度某档位的文案:优先表内,缺失回退通用标签 — resolveText"""
